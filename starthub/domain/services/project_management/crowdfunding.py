...
from domain.exceptions.permissions import UpdateDeniedPermissionException, DeleteDeniedPermissionException
...
class ProjectCrowdfundingService(AbstractDomainService):
    ...
    def create(self, payload: ProjectCrowdfundingCreatePayload) -> ProjectCrowdfunding:
        crowdfunding =  self._write_repository.create(data=payload)
        logger.debug("Crowdfunding created.")
        return crowdfunding
    ...
    def _check_delete_permissions(self, user: User, crowdfunding: ProjectCrowdfunding) -> None:
        ...
        delete_own_permission = self._permission_service.create_permission_vo(
            model=ProjectCrowdfunding, action=ActionEnum.DELETE, scope=ScopeEnum.OWN,
        )
