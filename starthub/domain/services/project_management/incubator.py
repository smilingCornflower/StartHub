...
class IncubatorService(AbstractDomainService):
    ...
    def _check_update_permissions(self, user: User, incubator: ProjectIncubator) -> None:
        ...
        change_own_permission = self._permission_service.create_permission_vo(
            model=ProjectIncubator, action=ActionEnum.CHANGE, scope=ScopeEnum.OWN,
        )
        ...
        if has_permission:
            ...
            if project.creator == user:
                return None
        logger.exception(f"User with id = {user.id} does't have enough permissions to change incubator with = {incubator.id}.")
